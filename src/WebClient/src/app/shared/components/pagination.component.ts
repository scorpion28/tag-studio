  ...
  template: `
    ...
    <div class="flex items-center flex-column flex-wrap md:flex-row justify-between pt-4">
      ...
      <div class="inline-flex mt-2 xs:mt-0 items-center">
        ...
        <div class="pl-2">
          <button
            class="px-3 h-8 text-sm font-medium  rounded-s bg-blue-600 border-gray-700 text-alpha-81 hover:bg-blue-700  disabled:bg-dark-gray-350"
            (click)="pageChange.emit(pagination().pageNumber - 1)"
            [disabled]="!pagination().hasPreviousPage">
            Prev
          </button>
          <button
            class="px-3 h-8 text-sm font-medium  border-0 border-s rounded-e bg-blue-600 border-gray-700 text-alpha-81 hover:bg-blue-700 disabled:bg-dark-gray-350"
            (click)="pageChange.emit(pagination().pageNumber + 1)"
