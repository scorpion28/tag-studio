...
public class TagStudioFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
    ...
    public async Task<AppUser> CreateUserAsync(Guid userId, string? email = null, string? password = null)
    {
    ...
    public HttpClient CreateAuthorizedClient(Guid? userId = null, string? userEmail = null)
    {
        ...
        var user = new AppUser { Id = userId.Value.ToString(), Email = userEmail };

        using var scope = Services.CreateScope();

        // Generate tokens
        var tokenService = scope.ServiceProvider.GetRequiredService<JwtTokenProvider>();
        var token = tokenService.Create(user).AccessToken;
